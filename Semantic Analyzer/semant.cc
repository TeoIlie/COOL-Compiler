...
void formal_class::install_formal(EnvironmentP env)
{
    ...
    // ensure that the type declaration is not SELF_TYPE
        env->semant_error(this) << "Formal parameter " << name << " cannot have type SELF_TYPE.\n" << endl;
    else {
        if (! env->lookup_class(type_decl))
    ...
    if (name == self) {
    // ensure the name is different from self
...
{
    //similarily to inst_const_class, we can be sure the type is Bool
    type = Bool;
    return Bool;
}
...
{
    //similarily to inst_const_class, we can be sure the type is String
    type = Str;
    return Str;
}
...
Symbol sub_class::tc(EnvironmentP env)
{
    ...
    if ((e2->get_type() != Int) || (e1->get_type() != Int))
        env->semant_error(this) << "Non-Integer arguments: " << e1->get_type() << " - "
            << e2->get_type() << endl;
...
Symbol neg_class::tc(EnvironmentP env)
{
    ...
    if(e1->get_type() != Int)
        env->semant_error(this) << "Argument of '~' has type " 
...
Symbol lt_class::tc(EnvironmentP env)
{
    ...
    if ((e2->get_type() != Int) || (e1->get_type() != Int))
        env->semant_error(this) << "Non-Integer arguments: " 
...
Symbol comp_class::tc(EnvironmentP env)
{
    ...
    // return Bool after making sure it is a Bool
    type = Bool;
    return Bool;
}
...
Symbol object_class::tc(EnvironmentP env)
{
    if (env->var_lookup(name))
        ...
        env->semant_error(this) << "Identifier is undeclared: " << name << "." << endl;
        type = Object;
    }
    // return the type from the var_lookup table
    return type;
}
...
{
    Symbol temp2 = e2->tc(env);
    Symbol temp1 = e1->tc(env);
    
    if ((temp1 != temp2) && ((temp1 == Int)  || (temp2 == Int) || (temp1 == Bool) || 
        (temp2 == Bool) || (temp1 == Str)  || (temp2 == Str)))
        env->semant_error(this) << "Illegal comparison with a simple type." << endl;
...
{
    if (name == self) {
        env->semant_error(this) << "Not possible to assign to 'self'." << endl;
    }
...
{
    Symbol expression_type = expr->tc(env);
    for(int i = actual->first(); actual->more(i); i = actual->next(i)) {
        actual->nth(i)->tc(env);
    }
    ...
    if(type_name == SELF_TYPE) {
        // cannot be SELF_TYPE
        env->semant_error(this) << "Cannot static dispatch to SELF_TYPE." << endl;
        type = Object;
        return Object;
    }

    InheritanceNode *undefined = env->lookup_class(type_name);

    if(!undefined) {
        // class must be defined or else there is an error.
        env->semant_error(this) << "Static dispatch to not defined class " <<
                                type_name << "." << endl;
        type = Object;
        return Object;
    }

    if(! env->type_leq(expression_type, type_name)) {
        //ensure expression conforms to static dispatch type
        env->semant_error(this) << "Expression type " << expression_type 
            << " does not conform to declared static dispatch type " << type_name << "." << endl;
        type = Object;
        return Object;
    }

    method_class *method = undefined->method_lookup(name);

    if(! method) {
        env->semant_error(this) << "Static dispatch to not defined method " << name << "." << endl;
        type = Object;
        return Object;
    }

    if(actual->len() != method->num_formals())
        env->semant_error(this) << "Method " << name << " invoked with wrong number of arguments." << endl;
    else {
        for(int i = actual->first(); actual->more(i); i = actual->next(i))
            if (! env->type_leq(actual->nth(i)->get_type(), method->sel_formal(i)->get_type_decl()))
                env->semant_error(this) << "In the call of method " << name << ", type " 
                    << actual->nth(i)->get_type() << " of parameter " << method->sel_formal(i)->get_name() 
                    << " does not conform to declared type " << method->sel_formal(i)->get_type_decl() << "." << endl;

    }
    type = (method->get_return_type() == SELF_TYPE) ? expression_type :
...
Symbol cond_class::tc(EnvironmentP env)
{
    // check conditionals
    if(pred->tc(env) != Bool)
        env->semant_error(this) << "The predicate of if does not have type Bool." << endl;
...
Symbol loop_class::tc(EnvironmentP env)
{
    // check loops
    if(pred->tc(env) != Bool)
        env->semant_error(this) << "The loop condition does not have type Bool." << endl;
...
{
    // method for case statements
    type = No_type;
    expr->tc(env);

    for (int i=cases->first(); cases->more(i); i = cases->next(i))
    {
        Case c_curr = cases->nth(i);
        for(int j=cases->first(); j<i; j = cases->next(j))
        {
            if(cases->nth(j)->get_type_decl() == c_curr->get_type_decl())
            {
                env->semant_error(c_curr) << "Duplicate branch " << c_curr->get_type_decl() <<
        ...
        if (! env->lookup_class(c_curr->get_type_decl()))
            env->semant_error(c_curr) << "Class " 
                << c_curr->get_type_decl() << " of case branch is not defined." << endl;

        if (self == c_curr->get_name())
            env->semant_error(c_curr) << "'self' bound in 'case'." << endl;

        if (SELF_TYPE == c_curr->get_type_decl())
            env->semant_error(c_curr) << "Identifier called " 
                << c_curr->get_name() << " declared with type SELF_TYPE inside case branch." << endl;

        env->var_add(c_curr->get_name(), c_curr->get_type_decl());

        type = env->type_lub(type, c_curr->tc(env));
        env->var_exitscope();
    }
    return type;
}
